...
## EXP-003: 三阶段 Pipeline (A+C 方案) 🔄 进行中

**日期**: 2026-02-12

**目标**: 解决 LLM 提取 "mentions" 而非 "knowledge" 的根本问题

### 问题分析

简单的"不要提取文件名"规则无法泛化，因为：
- 如果文档是讲"文件命名规范"呢？文件名就是核心概念了
- 规则会越加越多，无法用 decision tree 判断何时适用

**根本问题**: LLM 不理解什么是"值得建模的知识"

### 解决方案：三阶段 Pipeline

```
Phase 1: First-Pass (文档理解)
├── 输入: 整个文档
├── 任务: 识别"这篇文档想教什么"
└── 输出: concept_candidates[], document_theme

Phase 2: Guided Extraction (引导式抽取)
├── 输入: chunk + First-Pass context
├── 任务: 在 chunk 中找 First-Pass 识别的概念
└── 约束: 只抽取有定义/解释的概念

Phase 3: Grounding Verification (Grounding 验证)
├── 输入: extracted entities
├── 任务: 验证每个实体是否 grounded
├── Grounded: 文档定义/解释了它
├── Not Grounded: 仅仅"提到"
└── 输出: 过滤掉 not grounded 的实体
```
...
| 文件 | 描述 |
|------|------|
| `src/agents/first_pass_agent.py` | Phase 1: 文档理解 Agent |
| `src/agents/grounding_verifier.py` | Phase 3: Grounding 验证 Agent |
| `src/agents/entity_extractor.py` | Phase 2: 更新支持 First-Pass context |
| `src/pipeline/enhanced_pipeline.py` | 三阶段 Pipeline 编排 |
...
**目标**: 噪声实体率 <15% (可能已被 EXP-003 解决)

### EXP-005: 实体类型引导关系选择
**目标**: 边精确度 +30%

### EXP-006: Self-Consistency 验证
**目标**: 精确度 +15%
